...
define('POWERPRESS_VERSION', '9.10.3' );
...
function powerpress_rss2_head()
{
	...
	if( !defined('POWERPRESS_RAWVOICE_RSS') || POWERPRESS_RAWVOICE_RSS != false )
	{
        ...
        if (isset($Feed['podcast_guid']) && UUID::is_valid($Feed['podcast_guid']))
        ...
        if (isset($Feed['live_item']) && $Feed['live_item']['enabled'] == '1' && UUID::is_valid($Feed['live_item']['guid'])) {
            ...
            $startDate = new DateTime($startArr[0] . ' ' . $startArr[1], new DateTimeZone($liveItem['timezone']));
            $start = $startDate->format('c');

            $endArr = explode('T', $liveItem['end_date_time']);
            $endDate = new DateTime($endArr[0] . ' ' . $endArr[1], new DateTimeZone($liveItem['timezone']));
            $end = $endDate->format('c');
            echo "\t<podcast:liveItem status=\"$status\" start=\"$start\" end=\"$end\">" . PHP_EOL;
            echo "\t\t<title>".$liveItem['title']."</title>" . PHP_EOL;
            echo "\t\t<guid>".$liveItem['guid']."</guid>" . PHP_EOL;
...
function powerpress_rss2_item()
{
    ...
    if (!empty($EpisodeData['value_pubkey']) && !empty($EpisodeData['value_split'])) {
